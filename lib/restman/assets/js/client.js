...
function detect_click () {
	$( 'a' ).unbind().on('click', function (event) {
		...
		targ = element.data('modTarget')|| element.data('target');
		console.log(targ);
		switch(element.data('type')){
			case 'link':
				get_page(request,targ);
			break;
			case 'switch':
...
function detect_submit () {
	$('#form').unbind().on('submit', function (e) {
		e.preventDefault();
		form = $(this);
		submit_form(form.attr('action'),form.attr('method'),form.serialize(),form.data('type'),element.data('modTarget') || form.data('target'))
	});
}
